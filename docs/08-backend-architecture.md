...
- 메인 UI는 `POST /api/analyze`를 통한 리뷰 리포트 생성에 집중합니다.
- `POST /api/recommend`는 확장 포인트로 유지되며, 현재 기본 화면에서는 직접 노출하지 않습니다.
...
- endpoint는 유지되지만, 프론트 기본 UX는 추천보다 리뷰 리포트에 집중합니다.
- 따라서 backend 아키텍처상 recommend는 제거 대상이 아니라 후속 확장용 API로 봅니다.
