...
- PAT는 브라우저 메모리에서만 유지하고 영구 저장하지 않음
...
1. 사용자가 레포 URL과 공개/비공개 모드를 선택한다.
2. private 레포라면 Username과 PAT를 입력한다.
3. 프론트가 GitHub API로 레포, 브랜치, workflow source를 조회한다.
4. 프론트가 workflow map을 만들고, 명시 관계와 추론 흐름을 함께 렌더링한다.
5. 사용자가 workflow를 선택하면 Job Graph, Run History, Workflow Source를 본다.
6. 프론트가 브랜치 단위 CI 리뷰 리포트를 조합한다.
7. 백엔드 분석 결과와 heuristic finding을 합쳐 리포트와 drawer에 반영한다.

### 8.2 예외 흐름

- PAT 누락: private repo 접근 차단 및 public 모드 전환 안내
